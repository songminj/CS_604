...
### 라우터
- 네트워크 계층의 장비로 네트워크 계층의 핵심 기능을 담당
- 홉: hop. 패킷이 호스트와 라우터 간, 혹은 라우터와 라우터 간 이동하는 과정. 즉, 패킷은 여러 홉을 거쳐 라우팅됨

### 라우팅 테이블
- 라우팅의 핵심으로, 특정 수신지까지 도달하기 위한 정보를 명시한 일종의 표와 같은 정보
- 환경에 따라 가지는 정보가 다르지만, 공통적으로 아래 정보를 가짐
  - 수신지 IP 주소와 서브넷 마스크
  - 다음 홉( = 게이트웨이)
  - 네트워크 인터페이스: 패킷을 내보낼 통로
  - 메트릭: 해당 경로로 이동하는 데에 드는 비용

- 디폴트 라우트: 기본적으로 패킷을 내보낼 경로. 라우팅 테이블에 엇는 경로로 패킷을 전송해야 할 때 사용
...
동적 라우팅
- 네트워크 규모가 커지고 관리할 라우터가 늘어나면 정적 라우팅 방식은 한계가 있음
- 자동으로 라우팅 테이블 항목을 만들고, 이를 이용해 라우팅하는 방식
- 라우팅 프로토콜을 활용해 라우터낄치 정보를 교환하며, 이를 통해 최적의 경로를 찾을 수 있음

AS
- Autonomous System. 동일한 라우팅 정책으로 운용되는 라우터들의 집단 네트워크
- AS마다 인터넷상에서 고유한 AS 번호가 할당됨
- AS 내에는 다수의 라우터가 있으며, 라우터들은 AS 내부에서만 소통할 수도 있고, AS 외부와 소통할 수도 있음
- AS 외부와 통신할 경우 AS 경계에서 AS 내외로 통신을 주고받을 수 있는 **AS 경계 라우터**를 이용

### 라우팅 프로토콜
- 라우터끼리 자신들의 정보를 교환하며 패킷이 이동할 최적의 경로를 찾기 위한 프로토콜
- AS 내부에서 수행되면 IGP(Interior Gateway Protocol), AS 외부에서 수행되면 EGP(Exterior Gateway Protocol)이라고 함
- IGP는 RIP와 OSPF, EGP는 BGP가 있음

**IGP: RIP와 OSPF**
- RIP: Routing Information Protocol. OSPF: Open Shortest Path First.
- 최적의 경로를 선정하는 과정에서 RIP는 거리 벡터가, OSPF는 링크 상태가 사용됨
- 거리 벡터: 거리를 기반으로 최적의 경로를 찾는 라우팅 프로토콜. 거리는 패킷이 경유한 라우터의 수를 의미.
  - RIP는 주기적으로 인접한 라우터끼리 정보를 교환하며 라우팅 테이블을 갱신하며, 특정 라우터까지 홉 수를 앎
- 링크 상태: 라우터끼리의 연결 상태를 그래프로 파악해 라우터들의 연결 관계, 연결 비용 등을 저장(= 링크 상태 데이터베이스 LSDB)
  - OSPF는 최적의 경로를 결정하기 위해 대역폭을 기반으로 메트릭을 계산. 대욕폭이 높을수록 메트릭이 낮다고 인식
  - 다만, 네트워크 구성이 매우 커지면 모든 정보를 관리하기 어렵기에 AS를 **에어리어** 단위로 나누고, 에어리어 내에서만 링크 상태를 공유
  - 에어리어 경계에 있는 라우터를 ABR(Area Border Router)라고 하며, 에어리어 간 연결을 담당

**EGP: BGP**
- BGP: Border Gateway Protocol. AS 간의 통신에서 사용되는 대표적인 프로토콜
- AS 간 통신(eBGP), AS 내 라우터 간 통신(iBGP) 모두 가능